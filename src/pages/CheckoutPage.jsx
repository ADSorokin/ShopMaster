...
const CheckoutPage = ({ cart, orderForm, setOrderForm, appliedCoupon, completeOrder, setCurrentPage, language, currency, formatPrice, getCartTotals }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-3xl font-bold mb-8">{language === 'ru' ? 'Оформление заказа' : 'Checkout'}</h1>
        ...
        {[1, 2, 3].map((step) => (
          <div key={step} className="flex items-center">
            ...
            <span className={`ml-2 font-medium ${
              ...
              {step === 1 ? getTranslation('delivery') : step === 2 ? getTranslation('payment') : getTranslation('confirm')}
            </span>
            {step < 3 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <div className="space-y-4">
                <label className="flex items-center space-x-3 p-4 border rounded-lg cursor-pointer">
                  ...
                  />
                  <div>
                    <div className="font-medium">{language === 'ru' ? 'Курьерская доставка' : 'Courier delivery'}</div>
                    <div className="text-sm text-gray-600">{language === 'ru' ? '1-3 дня, 500 ₽' : '1-3 days, 500 ₽'}</div>
                  </div>
                </label>
                ...
                <label className="flex items-center space-x-3 p-4 border rounded-lg cursor-pointer">
                  ...
                  />
                  <div>
                    <div className="font-medium">{language === 'ru' ? 'Самовывоз' : 'Pickup'}</div>
                    <div className="text-sm text-gray-600">{language === 'ru' ? 'Бесплатно, сегодня' : 'Free, today'}</div>
                  </div>
                </label>
              </div>
              ...
              {orderForm.delivery.type === 'pickup' && (
                <div className="mt-6">
                  <h3 className="font-semibold mb-4">{language === 'ru' ? 'Выберите пункт самовывоза' : 'Select pickup point'}</h3>
                  <div className="space-y-4">
                    {pickupPoints.map(point => (
                      <div key={point.id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div>
                          ...
                          <div className="text-sm text-green-600">{language === 'ru' ? 'Открыто до' : 'Open until'} {point.closingTime}</div>
                        </div>
                        <button className="text-indigo-600 text-sm">{language === 'ru' ? 'Выбрать' : 'Select'}</button>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <div className="space-y-4">
                <label className="flex items-center space-x-3 p-4 border rounded-lg cursor-pointer">
                  ...
                  />
                  <div>
                    <div className="font-medium">{language === 'ru' ? 'Банковской картой' : 'By card'}</div>
                    <div className="text-sm text-gray-600">{language === 'ru' ? 'Безопасная оплата' : 'Secure payment'}</div>
                  </div>
                </label>
                ...
                <label className="flex items-center space-x-3 p-4 border rounded-lg cursor-pointer">
                  ...
                  />
                  <div>
                    <div className="font-medium">{language === 'ru' ? 'Наличными' : 'Cash'}</div>
                    <div className="text-sm text-gray-600">{language === 'ru' ? 'При получении' : 'Upon delivery'}</div>
                  </div>
                </label>
              </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <div className="space-y-4">
                <div className="p-4 bg-gray-50 rounded-lg">
                  ...
                  <p>{orderForm.delivery.type === 'courier' ? (language === 'ru' ? 'Курьерская доставка' : 'Courier delivery') : (language === 'ru' ? 'Самовывоз' : 'Pickup')}</p>
                  {orderForm.delivery.type === 'courier' && (
                    <>
                ...
                <div className="p-4 bg-gray-50 rounded-lg">
                  ...
                  <p>{orderForm.payment.method === 'card' ? (language === 'ru' ? 'Банковской картой' : 'By card') : (language === 'ru' ? 'Наличными' : 'Cash')}</p>
                  {orderForm.payment.method === 'card' && (
                    <p>{language === 'ru' ? 'Карта:' : 'Card:'} **** **** **** {orderForm.payment.cardNumber.slice(-4)}</p>
                  )}
                </div>

                <div className="p-4 bg-gray-50 rounded-lg">
                  <h3 className="font-semibold mb-2">{language === 'ru' ? 'Товары' : 'Products'}</h3>
                  {cart.map((item) => (
                    <div key={item.id} className="flex justify-between text-sm">
                      <span>{typeof item.name === 'object' ? item.name[language] : item.name} x{item.quantity}</span>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-lg font-bold mb-4">{language === 'ru' ? 'Ваш заказ' : 'Your order'}</h3>
            ...
            {cart.map((item) => (
              <div key={item.id} className="flex items-center space-x-3">
                ...
                <div className="flex-1">
                  <h4 className="text-sm font-medium">{typeof item.name === 'object' ? item.name[language] : item.name}</h4>
          ...
          <div className="flex space-x-4 mt-6">
            ...
            >
              {orderForm.step < 3 ? getTranslation('next') : getTranslation('completeOrder')}
            </button>
          </div>
        </div>
      </div>
