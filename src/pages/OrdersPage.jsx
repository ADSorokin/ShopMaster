/**
 * Страница "Мои заказы" — отображает историю заказов пользователя.
 *
 * @component
 * @param {Object} props - Входные параметры компонента
 * @param {Object|null} props.user - Данные авторизованного пользователя или `null`, если не залогинен.
 * @param {Array<Object>} props.orders - Список заказов пользователя.
 * @param {Function} props.setCurrentPage - Функция для навигации (например, переход на другую страницу).
 * @param {string} props.language - Текущий язык интерфейса ('ru' или 'en').
 * @param {string} props.currency - Текущая валюта (например, 'RUB').
 * @param {Function} props.formatPrice - Функция форматирования цены с учётом валюты и языка.
 *
 * @example
 * <OrdersPage
 ...
 * @description
 * Поведение:
 * - Если пользователь не авторизован — показывает сообщение и кнопку входа.
 * - Если нет заказов — отображает пустое состояние с призывом к покупкам.
 * - Иначе — рендерит список заказов с деталями: статус, доставка, оплата, товары.
 *
...
const OrdersPage = ({ user, orders, setCurrentPage, language, currency, formatPrice }) => {
     ...
     *
     * @param {string} key - Ключ локализации (например, 'orders', 'total')
        ...
        return (
            <div className="max-w-md mx-auto px-4 sm:px-6 lg:px-8 py-8 text-center">
                ...
                <p className="text-gray-600 mb-6">
                    {language === 'ru'
                        ? 'Пожалуйста, войдите в свой аккаунт, чтобы просматривать историю заказов.'
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">
                            {user.orders} {language === 'ru' ? 'заказов' : 'orders'}
                        </p>
                    </div>
                </div>
            </div>

            {/* Пустое состояние или список заказов */}
            {orders.length === 0 ? (
                // Нет заказов
                ...
                <div className="space-y-6">
                    {orders.map((order) => (
                        <div key={order.id} className="bg-white rounded-lg shadow-md p-6">
                            {/* Заголовок заказа: номер и статус */}
                  ...
                  {order.status === 'completed'
                      ...
                      <span>
                        {typeof item.name === 'object' ? item.name[language] : item.name}
                                            ...
                                            <span>
                        {item.quantity} {language === 'ru' ? 'шт.' : 'pcs.'}
                      </span>
                                        </div>
                                    ))}
                                </div>
