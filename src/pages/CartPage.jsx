/**
 * Страница корзины — отображает товары, управление количеством, применение промокодов и итоги.
 *
 * @component
 * @param {Object} props - Входные параметры компонента
 * @param {Array<Object>} props.cart - Текущая корзина с товарами, количеством и опциями
 * @param {Function} props.setCart - Функция обновления состояния корзины
 * @param {Object|null} props.appliedCoupon - Применённый промокод (с кодом, скидкой и т.д.)
 * @param {Function} props.applyCouponCode - Функция применения промокода по коду
 * @param {Function} props.setCurrentPage - Функция навигации (например, переход к "checkout")
 ...
 * @description
 * Особенности:
 * - Отображает список товаров с изображением, названием, опциями и ценой
 * - Поддерживает изменение количества (с кнопками + и -)
 * - Удаление товара из корзины
 * - Применение промокода
 * - Показ итоговой суммы с учётом скидки и доставки
 * - Адаптивная сетка: товары слева, итоги справа на десктопе
 * - Поддержка мультиязычного интерфейса
 * - Пустое состояние при пустой корзине
 */
const CartPage = ({
                      cart,
                      setCart,
     ...
     *
     * @param {string} key - Ключ локализации (например, 'cart', 'total')
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {cart.length === 0 ? (
                ...
                <span>
                  {getTranslation('subtotal')} ({cart.reduce((sum, item) => sum + item.quantity, 0)})
                </span>
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex space-x-2">
                                <input
                                    ...
                                    aria-label={getTranslation('coupon')}
                                />
                                <button
                                    onClick={() => {
